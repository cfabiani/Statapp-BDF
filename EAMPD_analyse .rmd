...
get_regression_results_PR <- function(dependent_var, independent_var) {
  model=lm(formula(paste(dependent_var, "~", independent_var)), data = PR)
  coef <- coef(model)[2]
...
```{r}
results_F1_PR <- sapply(vars_PR, function(var) get_regression_results_PR(var, "F1"))
results_F2_PR <- sapply(vars_PR, function(var) get_regression_results_PR(var, "F2"))
