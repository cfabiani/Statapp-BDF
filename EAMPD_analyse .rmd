...
acf(F1_diff, main="ACF de F1", lag.max=20)
acf(F2_diff, main="ACF de F2", lag.max=20)
pacf(F1_diff, main="PACF de F1")
pacf(F2_diff, main="PACF de F2")
```
...
```{r}
# Avec la méthode ChatGPT
arima_F2=auto.arima(F2_diff, d=1)
summary(arima_F2)
...
arima_F1=auto.arima(F1_diff, d=1)
summary(arima_F1)
...
res42=sarima(F2_diff, 4,1,2, details=FALSE)
```
Tous les arx sont significatifs à tous les seuils. 
